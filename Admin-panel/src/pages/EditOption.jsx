...
const EditOption = () => {
  ...
  useEffect(() => {
    const existingOption = options.find(opt => opt.id === Number(id));
    if (existingOption) {
      setTabId(existingOption.tabId);
      setName(existingOption.name);
      setDesc(existingOption.desc);
      setPrice(existingOption.price?.toString() || '0');
      setStock(existingOption.stock?.toString() || '0');
      setImagePreview(existingOption.img);
    } else {
      navigate('/options'); // Redirect if not found
    }
    ...
    <main className="flex-1 p-6 md:p-margin-page max-w-container-max mx-auto w-full">
      <div className="mb-stack-lg">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
          Modify the details of this customization option. Changes will reflect instantly on the storefront.
        </p>
      </div>
